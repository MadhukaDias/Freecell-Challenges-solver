...
class Node {
 public:
  ...
  const Tableau& GetTableau(int i) const { return tableau_[i]; }
  const Reserve& GetReserve() const { return reserve_; }
  const Foundation& GetFoundation(int i) const { return foundation_[i]; }
  ...
  */
  ...
  // Expose AutoPlay for step-by-step simulation
  void RunAutoPlay() { AutoPlay(); }

  // Expose move methods for step-by-step simulation
  void ApplyReserveToFoundation(int r) { ReserveToFoundation(r); }
  void ApplyReserveToTableau(int r, int t) { ReserveToTableau(r, t); }
  void ApplyTableauToFoundation(int t) { TableauToFoundation(t); }
  void ApplyTableauToReserve(int t) { TableauToReserve(t); }
  void ApplyTableauToTableau(int t1, int t2) { TableauToTableau(t1, t2); }

  Move last_auto_move() const { return last_auto_move_; }
